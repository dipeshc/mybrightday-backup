...
// exponential backoff with full jitter. It is intentionally a transport-layer
// wrapper rather than a per-call helper so that adding it to a client retries
// every outbound request without per-call-site changes.
package httpx
...
// (3 attempts, 1s base delay, 8s cap).
type Policy struct {
	MaxAttempts int
...
func (t *RetryTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		delay := backoffDelay(attempt, t.Policy.baseDelay(), t.Policy.maxDelay())
		if err != nil {
...
// backoffDelay returns BaseDelay<<attempt capped at MaxDelay, with full jitter.
